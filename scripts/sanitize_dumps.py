...
# Носитель раздатки — то, на что текст показывает. Отдельным списком, потому
# что входит сразу в два правила ниже.
HANDOUT_CARRIER = (
    r"(?:картинк\w*|изображени\w*|фотографи\w*|фото|схем\w*|рисунк\w*|рисунок"
    r"|коллаж\w*|постер\w*|плакат\w*|обложк\w*|раздатк\w*)"
)
# Указание на носитель: «слева на фото», «стрелка на рисунке», «на коллаже мы
# скрыли». Порядок слов в русском свободный, поэтому проверяется в обе стороны.
HANDOUT_CUE = (
    r"(?:слева|справа|сверху|снизу|стрелк\w*|скрыт\w*|скрыли|закрасили"
    r"|закраш\w*|вырезали|заклеили|замазали|удалили)"
)
# Раздатка называет себя по-разному, и якорное «^Перед вами» ловило только
# самую явную форму: обкатка 08.09.2026 нашла в «Классике» вопрос «смотреть на
# эту картинку» без картинки. Общее у всех правил — дейксис: текст показывает
# на то, что лежит перед игроком, и без этого вопрос не берётся.
#
# Правила односторонние по цене ошибки: потерять играбельный вопрос из 8866
# дешевле, чем показать вопрос, который нельзя взять. Известное пере-исключение
# перечислено в T21 — это анафора («изучала ЭТИ фотографии» о снимках,
# описанных выше) и «перед собой» внутри цитаты.
RE_HANDOUT = re.compile(
    ...
    + "|".join(
        (
            r"раздаточн",
            r"перед\s+вами",
            # «Вы видите перед собой…» — та же раздатка другими словами.
            r"вы\s+(?:видите|наблюдаете)|видите\s+перед\s+собой",
            # «[Раздаются три иероглифа.]», «на раздатке». Огласовка приставки
            # меняется — «раздаётся», но «РОЗданный вам отрывок», — и второй
            # формой раздатка называет себя чаще всего.
            r"разда(?:ются|ется|ётся|йте|ём|ем)\b|на\s+раздатке|\bроздан\w*",
            # Указательное слово перед носителем: «на этой картинке»,
            # «данное изображение», «с этого фото».
            r"\b(?:эт(?:а|у|ой|ом|о|от|ого|и|их)|данн\w+|розданн\w+)\s+"
            + HANDOUT_CARRIER,
            # «Чёрным прямоугольником скрыты…» — раздатка, не называющая себя
            # раздаткой.
            r"(?:чёрн|черн|сер)\w*\s+(?:прямоугольник|квадрат|полос)\w*\s+скрыт",
            # Носитель без указательного слова, но с указанием внутри него.
            HANDOUT_CARRIER + r"[^.!?]{0,40}\b" + HANDOUT_CUE,
            r"\b" + HANDOUT_CUE + r"[^.!?]{0,40}" + HANDOUT_CARRIER,
        )
    )
    + ")"
)
...
def exclusion_reason(question, answer, theme, raw_question=None, handout_image=None):
    ...
    if not handout_image:
        # По исходному тексту: инструкция чтецу («[Раздаются фотографии…]»)
        # бывает единственным признаком раздатки, а очистка её срезает.
        if RE_HANDOUT.search(raw):
