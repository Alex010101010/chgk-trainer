...
def test_handout_deixis_beyond_pered_vami():
    """Обкатка 08.09.2026: в «Классике» попался вопрос «смотреть на эту
    картинку» — а картинки нет. Якорное `^перед вами` ловит только самую явную
    форму раздатки; правило ищет дейксис в любой формулировке.

    Реализация, оставленная якорной, краснеет на всех восьми случаях."""
    handouts = [
        "Автор вопроса предлагает смотреть на эту картинку. Назовите ЕЁ двумя словами.",
        "Стандартное ОНО — перед вами. Назовите ЕГО тремя словами.",
        "Вопрос перед вами.",
        "[Раздаются фотографии газели и дивана.] Назовите город.",
        "Назовите фильм, в котором снялся человек слева на фото.",
        "Мы не спрашиваем, в какой стране можно увидеть данное изображение. Где?",
        "Вы видите перед собой заголовок веб-страницы. Назовите роман.",
        "На шуточном коллаже мы скрыли ПЕРВЫЙ и ВТОРОЙ. Назовите ПЕРВЫЙ.",
    ]
    for question in handouts:
        actual = exclusion_reason(question, "ответ", None)[0]
        check(actual == "handout", f"раздатка не опознана: {question[:45]!r} -> {actual}")

    # Правило односторонее, но не любое упоминание картинки — раздатка:
    # названное и описанное словами произведение играется без неё.
    kept = [
        ("На рисунке Жана Эффеля бог создаёт облака из НИХ. Назовите ИХ.",
         "названный рисунок, описанный словами, — не раздатка"),
        ("Художник-портретист должен видеть перед собой человека, а что мы "
         "можем увидеть на картине анималиста?",
         "«видеть перед собой» в теле фразы — не «перед вами»"),
    ]
    for question, why in kept:
        actual = exclusion_reason(question, "ответ", None)[0]
        check(actual is None, f"{why}: -> {actual}")
...
def main():
    for test in [
        ...
        test_handout_deixis_beyond_pered_vami,
